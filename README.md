...
| **Active — next GitHub function wave** | Verify artifact attestations cryptographically, then manage Actions caches | The active verifier separates the downloaded ZIP's transport digest from its contained subjects, safely inventories bounded regular-file entries, and verifies only explicitly selected bytes against a fixed SLSA v1/source/signer policy. Its fixed hidden `gh` runtime, rehash boundary, cancellation/cleanup, normalized results, and opaque IPC are done; selected-account orchestration, the review/result UI, and production geometry evidence remain. Follow it with paginated cache inventory, key/ref filters, size/age context, and separately confirmed selected/key/all deletion. Bounded Pull Request Center and Issue Hub read waves follow; no command, API-path, or GraphQL editor is planned |
...
- **Active — selected-account orchestration:** connect the store to the repository-selected account's bounded bundle request, invalidate operations on account/repository/download/policy changes, and inject any selected credential only inside the main process. GitHub.com can verify a local bundle without an account token once trust material is available. GHE.com verification currently returns **Unavailable** because GitHub CLI 2.96 still resolves the trust domain in bundle mode; it remains unavailable until the selected GHE.com credential is injected main-process-only. Tokens remain excluded from IPC, React state, logs, and Temp files.
...
- **Active — remaining orchestration and UI adversarial tests:** safe subjects, the projected parser, opaque IPC/client abort races, fixed argv/environment, exact policy-failure classification, output/deadline limits, whole-service concurrency, process cancellation, shutdown gating, archive-change precedence, and Temp cleanup are covered. Store/account-change and stale-result invalidation plus React focus, accessibility, responsive layout, and normalized outcome rendering remain with their implementation slices.
