...
- **Active — safe subject inventory:** record the archive transport SHA-256 separately; inventory at most 2,000 structurally safe regular-file ZIP entries; issue opaque sender-scoped inventory/entry identities; extract and rehash only the selected bytes; and retain archive-level verification only when the archive digest is itself an attested subject.
