...
| **Active** | Expand named GitHub functions on a hardened transport | GitHub.com REST versioning, credential/header precedence, the bounded/cancellable multi-account Notifications inbox, and guided native Issue creation are done; the first native pull-request creation slice is implemented at `af5fd32078` with hardening at `853c3c3d7d` and one exact-remote follow-up active, while the first Actions artifact browse/download/digest slice is pushed at `cec14273a1` with reviewed transport/account corrections active. The fresh off-screen UI gate follows those corrections; PR metadata/lifecycle, artifact pagination beyond the first 100, effective branch rules, and Releases/assets remain sequenced |
...
| **P0 — verification pending** | Deepen a shallow repository's history; create a pull request through the first native compose/review/submit slice; browse and safely download a workflow run's first bounded artifact page with local digest and attestation-presence context | **History deepening is pushed at `462b0c7281`; native PR creation is pushed at `af5fd32078` plus hardening `853c3c3d7d`; Actions artifacts are pushed at `cec14273a1` with 37 focused tests independently rerun. Exact-remote PR and trusted-transfer/account corrections remain active, then all require the fresh production/off-screen UI gate before Done** |
| **P0 — active next** | Finish native-create exact-remote and Actions trusted-transfer/account corrections; enrich PR creation with templates/reviewers/assignees/labels; review, update, and merge a pull request; add interactive Actions artifact pagination beyond the first 100; inspect the effective branch rules for the current branch | **In progress** |
