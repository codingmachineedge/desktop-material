...
| **Active** | Expand named GitHub functions on a hardened transport | Native pull-request compose/review/create, Actions artifact download with local digest and attestation-presence context, and effective branch-rules inspection passed their production UI gate at `9e946fd527`. Server-filtered Actions run pagination is now implemented with explicit load-more, exact-account routing, cancellation, de-duplication, and polling that preserves older loaded pages; its 50/50 focused API/store/React/style checks pass. Artifact/job pagination, cryptographic attestation verification, a Pull Request Center, an Issue Hub, and a Release Manager follow |
...
| **P0 — Actions run pagination** | Load later workflow-run pages through a named app control; apply workflow/branch/event/status filters at the provider; preserve loaded older pages across polling; cancel stale requests; de-duplicate shifted pages; contain the pager at narrow widths | **Implemented; 50/50 focused checks pass. Exact production UI interaction and screenshot verification will run with the companion artifact-page checkpoint before this row moves to Done** |
| **P0 — active product wave** | Artifact and job pagination; cryptographic attestation verification; Pull Request Center; Issue Hub; Release Manager; Actions approvals and caches | **In progress** |
