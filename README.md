...
| **Active** | Expand named GitHub functions on a hardened transport | Native pull-request compose/review/create, Actions artifact download with local digest and attestation-presence context, and effective branch-rules inspection passed their production UI gate at `9e946fd527`. Server-filtered run pagination and compact artifact pagination are production-verified at `0aca4420df`. Attempt-aware 50-job pages, retained retry and de-duplication, exact page-two log/re-run actions, pending deployment inspection and review history, bounded approve/reject comments, and separate confirmed fork-run approval are production-verified at `2f40d8949a`. The fixed cryptographic-verifier runtime, opaque IPC, authoritative repository/run-attempt metadata, exact source-ref resolver, attempt-bound result parser, and main-process credential-lease broker are complete; selected-account store orchestration and the result UI are active next, followed by the smaller Actions cache manager, bounded Pull Request Center and Issue Hub read waves, and Release Manager |
...
| **Active — next GitHub function wave** | Verify artifact attestations cryptographically, then manage Actions caches | The active verifier separates the downloaded ZIP's transport digest from its contained subjects, safely inventories bounded regular-file entries, and verifies only explicitly selected bytes against a fixed SLSA v1/source/signer policy. Its fixed hidden `gh` runtime, rehash boundary, cancellation/cleanup, normalized results, opaque IPC, authoritative exact-attempt metadata, source-ref resolution, invocation binding, and credential-lease boundary are done; selected-account store orchestration, the review/result UI, and production geometry evidence remain. Follow it with paginated cache inventory, key/ref filters, size/age context, and separately confirmed selected/key/all deletion. Bounded Pull Request Center and Issue Hub read waves follow; no command, API-path, or GraphQL editor is planned |
...
- **Done — verifier trust contract and projected result parser:** verification now requires an authoritative full source ref, exact repository/source/signer identities and SHAs, GitHub-hosted runners, and exact public/private/internal visibility. A pure trust boundary maps only GitHub.com or one tenant GHE.com API endpoint to its canonical web host and OIDC issuer. Reusable workflow metadata accepts only a separately validated SHA/ref plus a matching safe `@sha`, `@full-ref`, or heads/tags ref-tail suffix; absent or mismatched metadata cannot invent a signer. The fixed `gh` JSON projection omits raw bundles and predicate bodies, caps output at 1 MiB / 30 records, validates every record and 1–8 timestamps, and preserves each verified attestation as separate normalized evidence. Nineteen new adversarial tests, the 37-test combined provider/trust regression, and the exact 1,017-byte live projection for run `29283111640` pass.
- **Done — fixed verifier runtime and IPC:** a main-process-only lease reopens and fully revalidates the archive, re-extracts and rehashes one subject, and keeps its private path valid only across the awaited fixed verifier call. The hidden `gh` process receives canonical bundle JSONL through an owned `0600` Temp file, closed stdin, fixed arguments and identity policy, a sanitized environment, two-layer concurrency caps, a 120-second deadline, 1 MiB stdout and 64 KiB internal stderr ceilings, exact cancellation, process-tree waiting, strict projected-result parsing, and cleanup that can defeat an otherwise successful result. The typed client sends only opaque download/inventory/entry/operation identities, the expected digest, canonical bundles, and normalized policy—never an executable, arguments, token, endpoint, archive path, or raw output. Thirty-six focused client, IPC, subject, runner, service, and shutdown tests and the 100-test combined artifact/provider regression plus full TypeScript pass. Local `--bundle` mode avoids attestation API fetching but can still initialize trust through Sigstore TUF mirrors, so it is not described as network-disconnected verification. A Windows Job Object remains a low-risk hardening follow-up for the unusual case where a trusted `gh` descendant inherits stdio after its parent exits.
- **Done — authoritative metadata and exact run-attempt binding:** purpose-named methods on one selected API instance now fetch the exact canonical repository/visibility and exact workflow-run attempt through the existing fatal 2 MiB Actions reader, drop unknown provider fields, and preserve repository/attempt/tag errors. A conservative source-ref resolver validates `head_branch` only as a lookup key, serially probes the fully encoded branch and tag names, requires the returned canonical ref, accepts direct branch/lightweight-tag commits or peels at most eight cycle-free annotated tags, and returns a ref only when exactly one namespace terminates at the attempt SHA. The normalized policy carries positive-safe `runId` and `runAttempt`; every projected attestation must contain the one byte-for-byte exact `/actions/runs/{run}/attempts/{attempt}` invocation. Sixty-one focused metadata, API, policy, result, runtime, and client tests, the 126-test combined artifact/provider/account regression, and full TypeScript pass. No cached/latest-run fallback, raw response, arbitrary URI, command, API path, credential, or local path enters the contract.
- **Done - selected-account credential lease broker:** a pure main-process registry accepts only a canonical current GHE.com endpoint/account-key shape plus bounded login and account generation, then issues a sender-, host-, and generation-bound random 128-bit one-use handle with a five-minute TTL and 4-per-sender / 32-global caps. Navigation, destruction, explicit release, expiry, shutdown, and every main `update-accounts` refresh revoke the exact active lease. The verifier claims a GHE.com handle before any verification await, rehashes the subject and prepares private canonical bundle JSONL before a bounded exact keychain read, injects `GH_TOKEN` only into the fixed GHE.com child environment (never its argv, the new provenance registration/verify/result IPC, provenance result data, provenance logs, or Temp data), and rereads the exact credential after the whole `gh` tree closes. Token removal, rotation, revocation, cross-tenant use, or a non-identical JS string suppresses a result; a cross-tenant mismatch consumes its one-use opaque handle but does not read or inject a token. GitHub.com and zero-bundle checks require a null handle and never read keychain credentials.
- **Active - selected-account store orchestration:** connect the repository-selected account, authoritative metadata reads, and bounded bundle request to safe registration/handle/release calls; increment and send account generations on repository/account/download/policy changes; and prevent stale results from reappearing. The broker is ready, but this checkpoint deliberately does not add a renderer/store caller, so the current product keeps GHE.com verification **Unavailable** until that named orchestration is complete. The new provenance registration/verify/result IPC and provenance UI state remain token-free; pre-existing account-management transport is outside this checkpoint.
...
- **Active — remaining orchestration and UI adversarial tests:** safe subjects, authoritative repository/attempt/ref/tag parsing, exact selected-API paths and error semantics, branch/tag ambiguity and annotated-tag peeling, exact invocation binding across every record, opaque IPC/client abort races, fixed argv/environment, exact policy-failure classification, output/deadline limits, whole-service concurrency, process cancellation, shutdown gating, archive-change precedence, and Temp cleanup are covered. Store/account-change and stale-result invalidation plus React focus, accessibility, responsive layout, and normalized outcome rendering remain with their implementation slices.
