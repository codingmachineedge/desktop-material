...
| **Done** | Add the first guided repository-function batch | Status summary, repository health, recent-signature audit, maintenance preview/run, reflog recovery, ZIP/TAR export from `HEAD`, and full-history bundle export use fixed safe recipes, purpose-built controls, confirmation, streaming results, exact cancel, native save/reveal, and repository refresh—without a raw command search/editor |
| **Active** | Expand audited Git capabilities as named functions | File history/blame and recent-signature audit are done; patch/bundle exchange, interactive rebase, shallow/sparse controls, restore-from-commit, signing setup, hooks, LFS, remotes, and deeper worktree administration follow |
...
|---|---|---|---|
| **Git** | Core repository, branch, commit, diff, rename-following file history/line blame, signature audit, stash, remote, worktree, merge, rebase, fetch/pull/push, automation, guarded cleanup, guided source-archive export, and portable full-history bundle export workflows | Patch exchange and bundle import/verify; signing setup; complete remote/worktree/stash administration; bisect, sparse/shallow, range-diff, notes, grep, and config tools | Fixed, audited Git recipes may power named app functions, but the UI exposes task-specific controls and results rather than a raw command list |
