...
| **Active — Actions run inspector** | Complete named job, attempt, and pending-run review functions | **Implementation and focused checks complete:** bounded 50-job pagination retains page one across later-page failure/retry and de-duplicates shifted pages; a 100-option selector plus bounded numeric jump keeps every valid attempt reachable; latest-attempt changes abort stale work and rebuild the selected historical pages; every loaded job keeps exact log and re-run controls; pending deployment/history reads, bounded approve/reject comments, and separate confirmed fork approval use the repository-selected account. Modal scrims consume input, contain focus, restore it on close, and narrow layouts stack recovery controls without sideways scrolling. TypeScript, targeted lint, responsive style contracts, and 124/124 focused Actions checks across 22 suites pass. **Remaining gate:** deterministic loopback-provider exercise, exact request receipts, production build, minimum/short/200%-requested geometry scans, inspected screenshots, wiki, and Pages publication |
