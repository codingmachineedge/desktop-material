...
Last updated: **July 13, 2026**. Detailed reproducible evidence lives in the [Git, GitHub, and GitKraken parity audit](.codex/run-manifests/2026-07-12-git-gh-interactive-audit.md), the [P0 production UI gate](.codex/run-manifests/2026-07-13-p0-production-ui-gate.md), and the [Actions pagination UI gate](.codex/run-manifests/2026-07-13-actions-pagination-ui-gate.md).
...
| **Active** | Expand named GitHub functions on a hardened transport | Native pull-request compose/review/create, Actions artifact download with local digest and attestation-presence context, and effective branch-rules inspection passed their production UI gate at `9e946fd527`. Server-filtered run pagination and compact artifact pagination are now production-verified at `0aca4420df` as explicit named load-more workflows with exact-account routing, cancellation, de-duplication, retained-page retry, and contained responsive pagers; the 74/74 focused artifact/run checks pass. Job pagination, pending-run approvals, cryptographic attestation verification, a Pull Request Center, an Issue Hub, and a Release Manager follow |
...
| **Active — Actions run inspector** | Complete named job, attempt, and pending-run review functions | Add bounded 50-job pagination with retained retry and de-duplication; select the current or an earlier run attempt; open logs and re-run a page-two job; inspect pending deployment environments; and confirm deployment approval/rejection or fork-run approval. The gate requires exact-account routing, stale-request cancellation, bounded comments, permission-aware errors, and zero page/dialog horizontal overflow, clipping, or overlap at the supported minimum and requested 200% scale |
...
| **P0 — active product wave** | Job pagination and attempt-aware inspection; pending-run approvals; cryptographic attestation verification; Pull Request Center; Issue Hub; Release Manager; Actions caches | **In progress** |
