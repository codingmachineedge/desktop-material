...
- **Active — contract and subject metadata:** record the archive transport SHA-256 separately; safely inventory bounded regular-file ZIP entries; let the user explicitly select one contained subject; extract and rehash only those selected bytes; and retain archive-level verification only when the archive digest is itself an attested subject. The contract is recorded, but parser, provider, and store implementation remain pending.
- **Queued — verifier and IPC:** add a fixed-argument, closed-stdin, cancellable verifier boundary with bounded bundle and output handling, selected-account routing, digest rechecks, trust-policy validation, exact-process cleanup, and normalized results. No executable, arguments, token, endpoint, or raw output crosses into the UI.
...
- **Queued — focused and adversarial tests:** cover ZIP traversal, absolute and duplicate paths, links and special entries, encryption, unsupported compression, size/ratio ceilings, extraction cancellation, changed bytes, malformed bundles/output, account changes, unsupported environments, stale responses, and the verified/unavailable/failed distinctions.
