...
| **Active** | Expand audited Git capabilities as named functions | File history/blame, restore-file-version, recent-signature audit, source archives, full-history bundle export/verification/import, guided shallow cloning, and sparse-checkout administration are done; patch exchange, interactive rebase, signing setup, hooks, LFS, remotes, and deeper worktree administration follow |
| **Active** | Expand named GitHub functions on a hardened transport | GitHub.com REST versioning, credential/header precedence, the bounded/cancellable multi-account Notifications inbox, and guided native Issue creation are done; PR review/merge, Releases, repository rules/settings, and deeper Actions administration follow |
...
| **Active — off-screen verification** | Build and interactively verify every changed UI off-screen | The exact MCP build command now passes against lockfile-matched JavaScript packages plus locally restored, version-identical GitHub Desktop N-API/helper artifacts; renderer/main/CLI/highlighter bundles, embedded Git, SASS validation, license metadata, and the unpackaged `out` tree all complete without a network fetch. Interactive regular/compact checks and pixel inspection are underway |
| **Active after build** | Refresh README, wiki, Pages, and screenshot evidence | Capture at desktop and compact widths, inspect original pixels for clipping/overlap/oversized text, record hashes, then publish the same verified state |

### Capability roadmap

| Area | Available now | Next native interactive milestones | Long-tail access |
|---|---|---|---|
| **Git** | Core repository, branch, commit, diff, rename-following file history/line blame, confirmed working-tree file-version restore, signature audit, stash, remote, worktree, merge, rebase, fetch/pull/push, automation, guarded cleanup, guided bounded-history shallow cloning, cone-mode sparse-checkout administration, guided source-archive export, portable full-history bundle export/verification, and guarded create-only branch import from inspected bundles | Patch exchange; signing setup; complete remote/worktree/stash administration; bisect, history deepening, range-diff, notes, grep, and config tools | Fixed, audited Git recipes may power named app functions, but the UI exposes task-specific controls and results rather than a raw command list |
| **GitHub CLI** | Native repository, pull-request reading, guided Issue authoring, multi-account Notifications triage, Actions browsing/logs/dispatch/rerun/cancel, account, organization, clone, fork, and publish foundations | PR authoring/review/merge; releases/assets; labels/milestones; rulesets/settings/collaborators; Actions artifacts/caches/runners/secrets/variables/environments; Projects and Discussions | `gh` can back provider-scoped functions internally; users interact with purpose-built forms, previews, confirmations, and results |
| **GitHub REST and GraphQL** | Account-scoped API layer, safe request/confirmation/redaction contracts, a bounded/cancellable Notifications inbox with conditional polling and pagination, and exact-provider native Issue creation | Version and permission awareness across more functions; PR review/merge; Releases; rate limits and bounded pagination; security, deployment, Pages, organization, team, and administration surfaces | The full schema is coverage evidence and an implementation checklist, not an endpoint browser; supported operations become named app features |
...
| **P1 Git** | Command palette; full interactive-rebase todo editor; shallow and sparse controls; signing settings; hooks, LFS, remotes, worktrees, patch exchange, and bundle import | **Restore-file, shallow-clone, sparse-checkout, and full bundle export/verify/import slices done; remaining functions planned** |
| **P1 GitHub** | Notifications; Issues; PR authoring/review/merge; Releases/assets; repository rules/settings/collaborators; Actions artifacts/caches/runners/secrets/variables/environments | **Notifications and guided Issue-creation slices done; remaining functions planned** |
