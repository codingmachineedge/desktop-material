...
sparse-checkout implementation and local acceptance are complete. The concise
public status and current maintenance gates live in
...
Appearance tab. The toolbar owner additionally provides localized text color
and complete bounded font controls. Repository typography is a partial layer
over its profile, and the adaptive toolbar remeasures whenever that layer
changes.
...
publication checks passed; exact receipts are recorded in `HANDOFF.md` and the
M23 publish-mode run manifest.
...
| **Word-style per-tab appearance** | **COMPLETE; LOCAL REGRESSION VERIFIED** | Right-clicking the actual tab label initializes that clicked tab—even when inactive—before opening its anchored editor. During startup or profile replacement, unavailable history stays non-throwing and localized retry guidance replaces the editor; profile/revision fencing ignores delayed prior-profile loads. Bold, italic, underline, size, font family, alignment, and independent foreground/background controls remain bounded, with an owner-local Git repository and inspect/undo/redo/restore history. See `app/src/lib/stores/repository-tabs-store.ts`, `app/src/ui/repository-tabs/repository-tab-strip.tsx`, and `app/styles/ui/_repository-tabs.scss`. |
...
| **Provider triage and organization Git account propagation** | **COMPLETE; VERIFIED LOCALLY** | Triage resolves the canonical `endpoint#id` binding saved by Repository Settings and reacts to repository replacement/binding updates without requiring a reopen. HTTPS fetch, pull, push, post-push refresh, scheduled sync, refspec fetch, and remote-HEAD discovery preserve that exact stable key through the credential helper; no token enters commands or environment variables. One usable write-capable same-origin identity may bind an unassigned repository; missing explicit bindings fail closed, and unrelated settings saves do not bind the first same-host identity. SAML recovery accepts GitHub's supported organization-quote forms. See `app/src/lib/stores/app-store.ts`, `app/src/lib/stores/git-store.ts`, `app/src/lib/repository-matching.ts`, and `app/src/ui/repository-settings/repository-settings.tsx`. |
