...
- Files over 100 MiB route through release-backed cheap LFS before every commit
  ...
  bucket holds at most 1,000 objects; atomic file/manual groups roll from
...
| CI update-coming-soon status and exact-SHA Release notes | **Local acceptance complete; remote verification pending** | The updater derives only the configured HTTPS GitHub feed, requires an exact newer main `Windows x64` job in either push CI or installer packaging for the same run ID/SHA, and accepts the status only when GitHub proves that SHA is ahead of the installed exact SHA. It keeps the state transient, localizes it in English/Cantonese/bilingual modes, and fences stale async results behind real updater events. The release workflow generates at most 50 sanitized commit subjects from the previous published tag through `RELEASE_TARGET_SHA`, then revalidates main/tag immediately before publishing that exact target through `body_path`. CI, installer, and Pages invocations have unique per-run concurrency groups so newer work cannot cancel or replace older running/pending work; source contracts enforce both uniqueness fields. Focused updater, i18n, workflow, and generator checks pass 35/35; app/script TypeScript pass. Integration, remote CI, and a published sample Release remain. |
